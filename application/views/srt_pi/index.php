...
Surat / Peraturan Pupuk Indonesia
    ...
    <section class="content">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">Surat / Peraturan Pupuk Indonesia</h3>
            </div>
            <div class="box-body">
                <div class="row">
                    <div class="col-md-12">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered table-pkc" id="table_surat_pi">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>Tahun</th>
...
<?php $this->load->view('partials/javascript'); ?>
    <script>
        ...
        function table_surat_pi_f() {
            table_surat_pi = $("#table_surat_pi").DataTable({
                processing: true,
                serverSide: true,
                searching: true,
                lengthMenu: [
        ...
        function create() {
            $.ajax({
                url: '<?= base_url('surat_pupuk_indonesia/create') ?>',
                success: function(response) {
                    bootbox.dialog({
                        title: 'Buat Dokumen Surat Pupuk Indonesia',
                        message: response
                    });
                }
            })
        ...
        function edit(id) {
            $.ajax({
                url: '<?= base_url('surat_pupuk_indonesia/edit') ?>/'+id,
                success: function(response) {
                    bootbox.dialog({
                        title: 'Edit Dokumen Surat Pupuk Indonesia',
                        message: response
                    });
                }
            });
        ...
        function store() {
            ...
            var formData = new FormData($("#form_create_srt_pi")[0]);
            $.ajax({
                url: '<?= base_url('api/srt_pi/store') ?>',
                type: 'POST',
                data: formData,
        ...
        function update(id) {
            ...
            $("div.form-group").removeClass('has-error');
            $("div.form-group span.help-block").remove();
            var formData = new FormData($("#form_edit_srt_pi")[0]);
            $.ajax({
                url: '<?= base_url('api/srt_pi/update') ?>/'+id,
                type: 'POST',
                data: formData,
                enctype: 'multipart/form-data',
                cache: false,
        ...
        function remove(id) {
            bootbox.confirm({
                ...
                callback: function (result) {
                    if(result) {
                        $.ajax({
                            url: '<?= base_url('api/srt_pi/destroy') ?>/'+id,
                            type: 'DELETE',
                            headers: { 'JWT': JWT }
                        }).done(response => {
