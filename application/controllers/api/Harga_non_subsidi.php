...
class Harga_non_subsidi extends PKCC_Controller {
        ...
        if($this->form_validation->run()){
            if (isset($_FILES['file']) && is_uploaded_file($_FILES['file']['tmp_name'])) {
                ...
                if ($upload['success']) {
                    ...
                    $target_add = 'harga_non_subsidi/'.$message['file_name'];
                    $sftp_file = $this->general->move_sftp($message, $target_add);
                    
                    $file = [
                        'file_name' => $message['file_name'],
                        'file_path' => $sftp_file,
                        'file_download_path' => base_url('api/harga_non_subsidi/download?path='.base64_encode('./upload/documents/harga_non_subsidi/'.$message['file_name']).'&name='.base64_encode($message['file_name'])),
    ...
    public function destroy($id)
    {
        ...
        $this->auth->user();
        try {
            $hns = $this->harga_non_subsidi_m->find($id);
            $file = $this->files_m->find($hns->file_id);
