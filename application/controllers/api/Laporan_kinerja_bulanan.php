...
class Laporan_kinerja_bulanan extends PKCC_Controller {
        ...
        if($this->form_validation->run()){
            if (isset($_FILES['file']) && is_uploaded_file($_FILES['file']['tmp_name'])) {
                ...
                if ($upload['success']) {
                    ...
                    $target_add = 'laporan_kinerja_bulanan/'.$message['file_name'];
                    $sftp_file = $this->general->move_sftp($message, $target_add);
                    $file = [
                        'file_name' => $message['file_name'],
                        'file_path' => $sftp_file,
                        'file_download_path' => base_url('api/laporan_kinerja_bulanan/download?path='.base64_encode('./upload/documents/laporan_kinerja_bulanan/'.$message['file_name']).'&name='.base64_encode($message['file_name'])),
    ...
    public function destroy($id)
    {
        ...
        $this->auth->user();
        try {
            $lkb = $this->laporan_kinerja_bulanan_m->find($id);
            $file = $this->files_m->find($lkb->file_id);
